...
- [x] Criar serviço de e-mail para eventos essenciais: confirmação de conta, reset de senha, pedido criado, pagamento aprovado, domínio ativado.
- [x] Critérios de aceite: e-mails mínimos são disparados e podem ser reprocessados em falha.
- [x] Notas técnicas: usar provider abstraído e templates simples versionados no código.
- [x] Dependências: autenticação, pedidos e domínios.
...
- [x] Padronizar logs com contexto de request, usuário, loja e módulo.
- [x] Critérios de aceite: erros e eventos operacionais críticos podem ser rastreados.
- [x] Notas técnicas: incluir correlation ID e mascarar dados sensíveis.
- [x] Dependências: configuração central da API.

### Task 13.6 (P1): Implementar tratamento global de erros

- [x] Criar filtros/interceptors para normalizar erros de API e falhas internas.
- [x] Critérios de aceite: frontend recebe respostas consistentes; backend registra detalhes técnicos.
- [x] Notas técnicas: mapear erros de domínio, validação e integração externa.
- [x] Dependências: API estruturada.

### Task 13.7 (P2): Configurar healthchecks e monitoramento básico

- [x] Expor healthchecks de API, banco, Redis e filas.
- [x] Critérios de aceite: ambiente permite validar rapidamente saúde operacional do sistema.
- [x] Notas técnicas: usar rota protegida ou segmentada para monitoramento.
- [x] Dependências: infraestrutura base.
