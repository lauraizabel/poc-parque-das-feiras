...
<p>O pagamento do seu pedido na loja <strong>${escapeHtml(storeName)}</strong> foi <strong>${statusLabel}</strong>.</p>
<div class="details">
<dl>
  <dt>Pedido</dt>
...
<p>Você pode acompanhar o pedido pelo link abaixo:</p>
<p><a href="${escapeAttr(`${getStoreUrl(storeName)}/orders/${orderId}?token=acessar`)}" style="color:#c45c2c">Ver pedido</a></p>`;
...
function getStoreUrl(storeName: string): string {
  return `https://${storeName.toLowerCase().replace(/\s+/g, "-")}.parquedasfeiras.com`;
}
